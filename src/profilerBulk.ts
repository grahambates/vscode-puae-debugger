...
//   [u32 gridLen][u32 chipLen][u32 slowLen][u32 customLen][grid][chip][slow][custom].
...
export function packBulk(raw: RawCapture): Uint8Array | undefined {
  ...
  const out = new Uint8Array(HEADER + grid.length + chip.length + slow.length + custom.length);
  ...
  out.set(custom, HEADER + grid.length + chip.length + slow.length);
  return out;
}
...
export function unpackBulk(buf: ArrayBuffer): { dma?: IDmaModel; dmaSnapshot?: DmaSnapshot } {
  ...
  const custom = new Uint8Array(buf, off, customLen);
  return {
